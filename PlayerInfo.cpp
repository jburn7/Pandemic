...
PlayerInfo::PlayerInfo(Player* player, const Vector2D &position, const float playerSpriteScale, const Outline& selectedIndicator) :
	Unit(position, (int)((player->getWidth()) * playerSpriteScale), (int)(player->getHeight() * playerSpriteScale)),
	mpPlayer(player),
	...
	mSelectedIndicator(selectedIndicator)
{
    ...
    mpTextBox = new TextBox(position + Vector2D(0, mpPlayer->getHeight() * mPlayerSpriteScale), Game::getInstance()->getUI().getFontSize(), Color(), "Role: TODO");
}
...
void PlayerInfo::draw()
{
	...
	if(mSelected)
	{
		const Vector2D outlineBounds = Vector2D((float)std::max(mpPlayer->getWidth(), mpTextBox->getWidth()), (float)std::max(mpPlayer->getHeight(), mpTextBox->getHeight()));
	...
	graphics.draw(mPosition, *mpPlayer->mConstantFrame, mShape, 0, Vector2D(mPlayerSpriteScale, mPlayerSpriteScale));
	mpTextBox->draw();
}
